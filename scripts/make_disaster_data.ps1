...
1..50 | ForEach-Object {
    "training run $_ log content" | Set-Content "apps/platform/logging/training/2026-05-10/run-$_.log"
}
Write-Host "  ✅ apps/platform/logging/ — 50 份训练日志"

Write-Host ""
Write-Host "🎬 灾难现场准备就绪。"
Write-Host "   总文件数:约 5650"
Write-Host "   总名义大小:约 2 GB(磁盘实际占用 < 100 MB,得益于稀疏文件)"
Get-ChildItem data/raw, runs, apps/platform/logging -Recurse -ErrorAction SilentlyContinue |
